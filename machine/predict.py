...
class YoloModel(TemplateModel):
    ...
    def predict(self, img, show=False):
        ...
        if self.device != None:
            results = self.model(img, device=self.device, verbose=False, task='detect', imgsz=self.img_size, max_det=5, iou=0.8)
        else:
            results = self.model(img, verbose=False, task='detect', imgsz=self.img_size, max_det=5, iou=0.8)
        result = results[0]
