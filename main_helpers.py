...
def first_run(predictor, warped_frame, SHOW_FRAME, corner_detection, selected_colors, targeting_method):
    ...
    if first_run_orientation and first_run_orientation["huey"] and first_run_orientation["enemy"]:
        ...
        display_angles(first_run_orientation, first_move_dictionary,
                       warped_frame, enemy_orientation=algorithm.enemy_orientation, enemy_future_position= algorithm.enemy_future_position,initial_run=True, centroids=corner_detection.centroids)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    else:
...
def display_angles(detected_bots_with_data, move_dictionary, image, enemy_orientation=315, enemy_future_position=np.array([0,0]),initial_run=False, is_recovering=False, is_backing=False, against_wall="", moving_forward=-1, is_flipped=False,  centroids=[], show=True, targeting_method = 1):
    if is_recovering:
